...
namespace DataModel
{
    ...
    public class VenueRoomDetailsV2 : IHasTagInfo, IMappingAware
    {
        ...
        //This ID is not autogenerated but in the POST/PUT Method this is checked
        public string? Id { get; set; }

        //Try to set the Shortname automatically
        public string? Shortname
        {
            get
            {
                ...
                if (Detail.TryGetValue("en", out var enDetail) && !string.IsNullOrEmpty(enDetail.Title))
                    return enDetail.Title;

                var first = Detail.Values.FirstOrDefault(d => !string.IsNullOrEmpty(d.Title));
                return first?.Title;
            }
        }
