                        ...
                        try
                        {
                            ...
                            if (bolzanoExists > 0)
                            {
                                isinsouthtyrol = await queryFactory
                                    .Query()
                                    .SelectRaw(
                                        $"ST_Contains((select st_transform(geometry, 4326) from geoshapes where name = 'Bolzano' limit 1), st_setsrid(st_makepoint(({gps.Longitude.GetValueOrDefault(0).ToString(culture)})::double precision, ({gps.Latitude.GetValueOrDefault(0).ToString(culture)})::double precision), 4326))"
                                    )
